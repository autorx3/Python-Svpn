...
class UDPHeader:
    def __init__(self,data, source_port = 4500, destination_port = 4500):
        self.source_port = source_port
        self.destination_port = destination_port
        self.length = 0  # Placeholder for length, will be updated when sending
        self.checksum = 0  # Placeholder for checksum, will be updated when sending
        self.length = len(data) + 8  # 8 bytes for UDP header
        self.payload = struct.pack('!HHHH', self.source_port, self.destination_port, self.length, self.checksum)
